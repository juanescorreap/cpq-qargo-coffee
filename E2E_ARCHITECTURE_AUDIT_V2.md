...
| **N6** | Seed nocturno solo en pg_cron (ausente en Supabase) → sin barrido completo, drift acumula. Asimetría con G1 (que SÍ dio mirror app-side al reaper, pero no al seed) | DB+Compute | 🟠 Mayor | `fn_seed_nightly_recompute` (fuente única) + tabla `calc_seed_runs` (claim exactly-once/día) llamada por pg_cron **y** worker app-side; chunked | ✅ |
...
- **N6** — implementado: migración 0026 (`fn_seed_nightly_recompute` + `calc_seed_runs`),
  worker `seed_nightly_recompute`/`seed_due`, llamada en `worker.py`. pg_cron pasa a ser
  `SELECT fn_seed_nightly_recompute('pg_cron')`. Exactly-once/día por claim atómico.
- **G5/G6/G8/G9** — siguen diferidos (requieren carga real / migración invasiva).

### Pendiente operativo (no de código)
- **Verificar el servicio worker en Railway.** `railway.json` arranca solo `web`; el
  `worker:` del Procfile necesita un servicio Railway separado. Sin él, la cola `calc_jobs`
  no se drena y el lazo async (incluido N6) no corre. Bloqueante para todo lo asíncrono.
